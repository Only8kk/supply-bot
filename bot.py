...
def make_chart_pdl(df, pdh, pdl):
    ...
    candle_width, wick_width = 0.5, 0.8
    fig_width, fig_height = 10,5
    fig, (ax_candle, ax_vol) = plt.subplots(2,1,figsize=(fig_width,fig_height),dpi=200, gridspec_kw={'height_ratios':[3,1]}, sharex=True)
    ...
    buf = io.BytesIO(); plt.tight_layout(pad=0); plt.savefig(buf, format="png", facecolor=fig.get_facecolor()); plt.close(fig); buf.seek(0)
    return buf
...
async def scan_market(context: ContextTypes.DEFAULT_TYPE, ignore_cooldown=False):
    ...
    for t in targets:
        symbol=t["symbol"]
        try:
            daily=exchange.fetch_ohlcv(symbol,"1d",limit=2); pdh=daily[0][2]; pdl=daily[0][3]
            signal_found=False
...
async def manual_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    add_subscriber(cid)
    if cid in LAST_MANUAL_SCAN and time.time()-LAST_MANUAL_SCAN[cid]<300:
        await update.message.reply_text("⏱ Please wait before using /scan again."); return
